# Auxiliary script, used by DensityProcessing.py
...
def densityPlot(yearList, dataList, dataType, limitsList, metric, maxCB, step, show = True, save = False, saveFolder = None):
    ...
    if (dataType == "global"):
        ...
        for count, data in enumerate(dataList):
            ...
            # Title
            ax.set_title(f"{yearList[repIndex]} - {yearList[repIndex + 1]} (Number of words: {len(data.index)})")
    ...
    if (dataType == "square"):
        ...
        if (sqDataList):
            ...
            for count, data in enumerate(sqDataList):
                ...
                ax = fig.add_subplot(len(sqDataList), 1, count + 1)
                ax.set_aspect("equal")
                ...
                text = "Number of words: " + str(len(data.index))
                ax.text(0.0, -0.20, text, transform=ax.transAxes)
        ...
        if (sqDataListDot):
            ...
            for count, data in enumerate(sqDataListDot):
                # Subplot configuration
                ax = fig.add_subplot(len(sqDataListDot), 1, count + 1)
                ax.set_aspect("equal")
            ...
            #cb_ax = fig.add_axes([0.1, 0.1, 0.8, 0.02])
            cbar = plt.colorbar(im, ticks = ticksLabel)
            labels = [str(x) for x in ticksLabel if x != ticksLabel[-1]]
            labels.append(str(ticksLabel[-1]) + "+")
